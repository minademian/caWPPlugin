 ...
 *
 * The argument should be in the form: Nds_Admin_Form_Demo\Namespace. The
...
spl_autoload_register( function( $class_name ) {
		...
		if ( false === strpos( $class_name, 'CA_Worldapi' ) ) {
			return;
		}
		...
		for ( $i = count( $file_parts ) - 1; $i > 0; $i-- ) {
			...
			if ( count( $file_parts ) - 1 === $i ) {
				...
				} else if ( strpos( strtolower( $file_parts[ count( $file_parts ) - 1 ] ), 'class' ) ) {
					// Grab the name of the interface from its qualified name.
					$class_name = explode( '_', $file_parts[ count( $file_parts ) - 1 ] );
